...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [H_Est, Support] = OMP_SMC(y,Epsilon,Pilot_pwr,Lbar,K)
R = y;
G = size(Epsilon,2);
H_Est = zeros(G,K);
...
for indx_subc = 1:K
    ...
    for ell =1: Lbar % threshold = sparsity level
        % Find AoD/AoA pair
        [~, indx_c] = max(abs(Epsilon'*R(:,indx_subc)));
        % Update AoD/AoA pair index
        Index = [Index indx_c]; 
        h = zeros(G,1);
        % Estimate channel gains by solving the Least Square problem
        h(Index,:) = 1/sqrt(Pilot_pwr)*pinv(Epsilon(:,Index)'*Epsilon(:,Index))*Epsilon(:,Index)'*y(:,indx_subc);
        % Update residual
        R(:,indx_subc) = y(:,indx_subc) - sqrt(Pilot_pwr)*Epsilon(:,Index)*h(Index,:);
    end
